<template>
	<div class="page" v-show="paginationConf.totalItems > 0">
		<!-- <a :class="{disabled: paginationConf.currentPage == 1}" @click="changeCurrentPage(1)"><span>首页</span></a> -->
		<a href="javascript:void(0);" :class="{disabled: paginationConf.currentPage == 1}" class="cn" @click="prevPage()"><span>上一页</span></a>
		<a href="javascript:void(0);" v-for="item in pageList" track-by="$index" :class="{active: item == paginationConf.currentPage}" @click="changeCurrentPage(item)"><span>{{item}}</span></a>
		<a href="javascript:void(0);" :class="{disabled: paginationConf.currentPage == paginationConf.numberOfPages}" class="cn" @click="nextPage()"><span>下一页</span></a>
		<!-- <a :class="{disabled: paginationConf.currentPage == paginationConf.numberOfPages}" @click="changeCurrentPage(paginationConf.numberOfPages)"><span>末页</span></a> -->
	</div>
</template>
<style scoped>
/*.page-list {
